...
function SpectatorJoinLobby(lobbyuid, useruid, callback) {
    ...
    if (!Lobbies[lobbyuid]) return false;
    if (!Registrar.GetUserByUID(useruid)) return false;
    if (!Lobbies[lobbyuid].allowspectators) return false;

    const user = Registrar.GetUserByUID(useruid);
    Lobbies[lobbyuid].spectators.push({
        uid: useruid,
        name: user.username
    });

    Logger.game(`LOBBY ${lobbyuid} USER ${useruid} (${user.username}) JOINED AS SPECTATOR`);

    callback(user, Lobbies[lobbyuid], 'spectator-join');
