...
function EmitGameReconnect(user, game) {
    const gameuser = game.players.find(i => i.uid === user.uid);
    const gameuserconnection = Game.Registrar.GetConnectionByUser(gameuser.uid);

    // Get the latest board state from the game states
    const currentBoardState = game.gamestates[game.gamestates.length - 1].boardtiles;

    io.to(gameuserconnection).emit('game-begin', {
        game: game,
        tileset: Dist.GetDist(game.locale).dist,
        gameuser: gameuser,
        boardState: currentBoardState // Send the current board state
    });
